...
logger = logging.getLogger("llmguard_qa.evaluations")
...
def run_prompt_sensitivity(
    ...
    try:
        ...
        logger.error(f"Error during sensitivity evaluation: {e}")
        err_msg = str(e).lower()
...
def run_prompt_consistency(
    ...
    try:
        ...
        logger.error(f"Error during consistency evaluation: {e}")
        err_msg = str(e).lower()
        if "429" in err_msg or "quota" in err_msg or "rate limit" in err_msg or "resource exhausted" in err_msg:
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
